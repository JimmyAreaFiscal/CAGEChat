...
def on_topic_router(state: AgentState) -> AgentState:
    ...
    if on_topic == 'sim':
        logging.info("on_topic_router: Question is on topic, routing to question_decomposition")
        return "question_decomposition"
    else:
        logging.info("on_topic_router: Question is not on topic, routing to off_topic_response")
