...
from frontend_py.components.connection import get_chat_stream
import sys, os 
...
if "chats" not in st.session_state:
    st.session_state.chats: Dict[str, Dict] = {}
...
with st.sidebar:
    ...
    if st.button("➕ Novo Chat") or st.session_state.current_chat is None:
        cid = str(uuid4())
        st.session_state.chats[cid] = {"checkpoint_id": None, "messages": []}
        st.session_state.current_chat = cid

    for cid, chat in st.session_state.chats.items():
        label = next((m["content"][:28] + "…" for m in chat["messages"] if m["origin"] == "user"), "(sem título)")
